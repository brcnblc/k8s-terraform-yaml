...
resource "kubernetes_cluster_role_binding" "metrics_server_auth_delegator" {
  ...
  depends_on  = [null_resource.module_depends_on]
  metadata {
    name = kubernetes_cluster_role.metrics_server[count.index].metadata.0.name
    labels = local.kubernetes_resources_labels
  }
  ...
  subject {
    kind = "ServiceAccount"
    name = kubernetes_service_account.metrics_server[count.index].metadata.0.name
    namespace = kubernetes_service_account.metrics_server[count.index].metadata.0.namespace
  }
}
...
resource "kubernetes_deployment" "metrics_server" {
  ...
  spec {
    ...
    template {
      ...
      spec {
        service_account_name = kubernetes_service_account.metrics_server[count.index].metadata.0.name
